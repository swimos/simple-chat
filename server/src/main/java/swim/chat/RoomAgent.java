...
/**
 * This is a dynamically created WebAgent which handles 
 * the chat messages and users for a room. One webAgent 
 * is created per room. A new WebAgent is created at the
 * time the first user accesses the room and will persist 
 * until the server is stopped. Chat rooms and messages
 * will not persist after a server restart.
 * 
 */
public class RoomAgent extends AbstractAgent {
   ...
   */
  private HashTrieMap<String, Uplink> uplinks = HashTrieMap.empty();

  /**
   * {@code CommandLane} used by swim clients to post new messages 
   * to the MessageList lane for this room.
   */
  ...
  /**
   * private method used to clean out chat messages
   * that are older then THRESHOLD_MILLIS
   */
